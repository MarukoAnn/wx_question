...
<uni-shadow-root class="vant-dist-uploader-index"><view class="van-uploader">
  <view class="van-uploader__wrapper">
    ...
    <block v-if="previewImage">
      <view v-for="(item,index) in (lists)" :key="item.index" class="van-uploader__preview">
        <image v-if="item.isImage" :mode="imageFit" :src="item.url || item.path" :alt="item.name || ('图片' + index)" class="van-uploader__preview-image" :style="'width: '+(computedPreviewSize)+'px; height: '+(computedPreviewSize)+'px;'" :data-url="item.url || item.path" @click="doPreviewImage"></image>
        <view v-else class="van-uploader__file" :style="'width: '+(computedPreviewSize)+'px; height: '+(computedPreviewSize)+'px;'">
        ...
        <van-icon name="delete" class="van-uploader__preview-delete" @click.native="deleteItem" :data-index="index"></van-icon>
      </view>
    </block>
    ...
    <block v-if="isInCount">
      ...
      <block v-else>
        ...
        <view class="van-uploader__upload" :style="'width: '+(computedPreviewSize)+'px; height: '+(computedPreviewSize)+'px;'" @click="startUpload">
