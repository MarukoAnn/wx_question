...
<uni-shadow-root class="vant-dist-skeleton-index"><view v-if="loading" :class="'custom-class '+(utils.bem('skeleton', [{animate}]))">
  <view v-if="avatar" :class="utils.bem('skeleton__avatar', [avatarShape])" :style="'width:' + avatarSize + ';height:' + avatarSize"></view>
  <view :class="utils.bem('skeleton__content')">
    <view v-if="title" :class="utils.bem('skeleton__title')" :style="'width:' + titleWidth"></view>
    <view v-for="(item,index) in ('row')" :key="item.index" :class="utils.bem('skeleton__row')" :style="'width:' + (isArray ? rowWidth[index] : rowWidth)"></view>
  </view>
</view>
